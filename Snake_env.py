...
class Snake:
    ...
    def __init__(self, width, height):
        ...
        self.STATE_ID = {self.HEAD_COLOR: 0.25, self.BODY_COLOR: 1,
                         self.BORDER_COLOR: 1, self.BLANK_COLOR: 0, self.APPLE_COLOR: 0.75}
        self.borderShape = {'x': 0, 'y': 0,
                            'w': self.screen.WIDTH, 'h': self.screen.HEIGHT}
        self.textPos = [self.borderShape.get('x') + self.borderShape.get(
            'w') - 2, self.borderShape.get('x') + self.borderShape.get('w')]
    ...
    def createGame(self, length):
        self.matrix = [0 for i in range(
            self.borderShape.get('w') * self.borderShape.get('h'))]
        self.body = []
    ...
    def generateStaticBorder(self, borderLoc):
        ...
        for i in range(x, x + w):
            ...
            self.matrix[(h - 1 + y) * self.screen.WIDTH +
                        i] = self.BORDER_COLOR
